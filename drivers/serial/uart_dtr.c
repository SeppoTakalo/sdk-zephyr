...
LOG_MODULE_REGISTER(uart_dtr, 4);
...
static int uart_dtr_irq_tx_complete(const struct device *dev);
...
static void uart_dtr_work_handler(struct k_work *work)
{
	...
	if (data->dtr_asserted && !data->uart_powered) {
		...
		if (ret == 0) {
			...
			LOG_DBG("UART powered on (app_rx_enable %d, user_cb %p)", data->app_rx_enable, data->user_callback);
#ifdef CONFIG_UART_ASYNC_API
		...
		LOG_DBG("DTR asserted, UART powered on");
	} else if (!data->dtr_asserted && data->uart_powered) {
#ifdef CONFIG_UART_ASYNC_API
		/* Disable RX quietly (suppress RX_DISABLED event) */
		if (data->rx_active) {
			int err_rx = uart_rx_disable(config->uart_dev);
			if (err_rx) {
				LOG_ERR("Failed to disable RX: %d", err_rx);
			}
		...
		LOG_DBG("DTR deasserted, UART powered off");
	}
}
...
static int uart_dtr_irq_tx_complete(const struct device *dev)
{
	...
	return uart_irq_tx_complete(config->uart_dev);
}
				      ...
				      void *cb_data)
{
	...
	uart_irq_callback_user_data_set(config->uart_dev, cb, cb_data);
}
#endif
...
static int uart_dtr_tx(const struct device *dev, const uint8_t *buf, size_t len, int32_t timeout)
{
	...
	int ret = uart_tx(config->uart_dev, buf, len, timeout);
	if (ret) {
...
static int uart_dtr_rx_enable(const struct device *dev, uint8_t *buf, size_t len, int32_t timeout)
{
	...
	data->rx_active = true;
	int ret = uart_rx_enable(config->uart_dev, buf, len, timeout);
	LOG_DBG("uart_dtr_rx_enable: %p, %zu (ret %d)", (void *)buf, len, ret);
	return ret;
}
...
static int uart_dtr_rx_buf_rsp(const struct device *dev, uint8_t *buf, size_t len)
{
	...
	if (!data->rx_active) {
		data->rx_active = true;
		int err = uart_rx_enable(config->uart_dev, buf, len, data->rx_timeout);
		if (err == -EBUSY) {
...
release:
	LOG_DBG("Releasing buffer, RX not active (dtr %d, app_rx_enable %d, rx_active %d)", data->dtr_asserted,
		data->app_rx_enable, data->rx_active);
	if (data->user_callback && buf) {
...
static int uart_dtr_init(const struct device *dev)
{
	...
	ret = pm_device_runtime_enable(config->uart_dev);
	LOG_DBG("Enabled runtime PM for parent UART device: %d", ret);

	/* Initially assert DTR to power on the UART */
	//uart_dtr_set_dtr(dev, true);

	LOG_DBG("(%p)UART DTR wrapper initialized for parent: %s", dev, config->uart_dev->name);
...
static const struct uart_driver_api uart_dtr_driver_api = {
	...
	.irq_tx_complete = uart_dtr_irq_tx_complete,
