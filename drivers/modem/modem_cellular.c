...
static void modem_cellular_event_dispatch_handler(struct k_work *item)
{
	...
	while (k_pipe_read(&data->event_pipe, &event, sizeof(event), K_NO_WAIT) == 1) {
...
static void modem_cellular_delegate_event(struct modem_cellular_data *data,
					  enum modem_cellular_event evt)
{
	k_pipe_write(&data->event_pipe, &evt, sizeof(evt), K_NO_WAIT);
...
static int modem_cellular_on_init_power_off_state_enter(struct modem_cellular_data *data)
{
	...
	if (MODEM_CMUX_PM_ENABLED && config->cmux_runtime_pm) {
		/* Re-acquire UART handle as CMUX is going to release it */
		/* We might need UART for shutdown-script */
