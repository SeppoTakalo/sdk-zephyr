...
void lwm2m_firmware_set_update_state(uint16_t obj_inst_id, uint8_t state)
{
	...
	LOG_DBG("Update state = %d for instance %d", update_state[obj_inst_id], obj_inst_id);
}
...
void lwm2m_firmware_set_update_result(uint16_t obj_inst_id, uint8_t result)
{
	...
	LOG_DBG("Update result = %d for instance %d", update_result[obj_inst_id], obj_inst_id);
}
