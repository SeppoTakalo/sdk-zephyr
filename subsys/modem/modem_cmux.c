...
#define MODEM_CMUX_IDLE_TIMEOUT			(K_SECONDS(CONFIG_MODEM_CMUX_IDLE_TIMEOUT))

#define MODEM_CMUX_EVENT_CONNECTED_BIT		(BIT(0))
#define MODEM_CMUX_EVENT_DISCONNECTED_BIT	(BIT(1))
#define MODEM_CMUX_EVENT_POWERSAVE_BIT		(BIT(2))
#define MODEM_CMUX_EVENT_WAKEUP_BIT		(BIT(3))
...
static void modem_cmux_tx_bypass(struct modem_cmux *cmux, const uint8_t data[static 1], size_t len);
...
static void modem_cmux_send_msc(struct modem_cmux *cmux, struct modem_cmux_dlci *dlci)
{
	...
	LOG_INF("modem_cmux_send_msc: Sending MSC command for DLCI %u, FC:%d RTR: %d DV: %d",
...
static void modem_cmux_on_msc_command(struct modem_cmux *cmux, struct modem_cmux_command *command)
{
	...
	if (dlci) {
		LOG_INF("MSC command received for DLCI %u, (%p) (%hhx FC:%d, RTC:%d, RTR:%d, DV:%d)", msc->dlci_address, dlci, (int)command->value[1], msc->signals.fc, msc->signals.rtc, msc->signals.rtr, msc->signals.dv);
...
static void modem_cmux_on_psc_response(struct modem_cmux *cmux)
{
	...
	if (MODEM_CMUX_PM_ENABLED && cmux->dev && pm_device_runtime_is_enabled(cmux->dev)) {
		pm_device_runtime_put_async(cmux->dev, K_NO_WAIT);
	}
}
...
static void modem_cmux_on_dlci_frame(struct modem_cmux *cmux)
{
	...
	case MODEM_CMUX_FRAME_TYPE_DM:
		...
		break;
	default:
		LOG_WRN("Unknown %s frame type (%d, DLCI %d)", "DLCI", cmux->frame.type, cmux->frame.dlci_address);
		break;
	}
}
...
static void modem_cmux_on_frame(struct modem_cmux *cmux)
{
	...
	if (is_powersaving(cmux)) {
		if (cmux->dev && pm_device_runtime_is_enabled(cmux->dev)) {
			pm_device_runtime_get(cmux->dev);
		}
	}
...
static void modem_cmux_process_received_byte(struct modem_cmux *cmux, uint8_t byte)
{
	...
	case MODEM_CMUX_RECEIVE_STATE_RESYNC:
		...
		if (byte == MODEM_CMUX_SOF) {
			...
			cmux->frame_header_len++;
			if ((is_powersaving(cmux) ||
			     (is_connected(cmux) && sys_timepoint_expired(cmux->t3_timeout))) &&
...
static void modem_cmux_runtime_pm_handler(struct k_work *item)
{
	...
	if (!cmux->runtime_pm || !MODEM_CMUX_PM_ENABLED) {
		return;
	}

	bool expired = sys_timepoint_expired(cmux->idle_timeout);
...
static void runtime_pm_keepalive(struct modem_cmux *cmux)
{
	if (cmux == NULL || !cmux->runtime_pm || !MODEM_CMUX_PM_ENABLED) {
		return;
	}

	cmux->idle_timeout = sys_timepoint_calc(MODEM_CMUX_IDLE_TIMEOUT);
	k_work_reschedule(&cmux->runtime_pm_work, MODEM_CMUX_IDLE_TIMEOUT);
}
...
static void modem_cmux_tx_bypass(struct modem_cmux *cmux, const uint8_t data[static 1], size_t len)
{
	...
	int ret = modem_pipe_transmit(cmux->pipe, data, len);
	if (ret < 0) {
		LOG_ERR("Failed to transmit byte (%d)", ret);
	}
}
...
static bool powersave_wait_wakeup(struct modem_cmux *cmux)
{
	...
	if (is_powersaving(cmux)) {
		LOG_WRN("Power saving mode, wake up first");
		...
		cmux->t3_timeout = sys_timepoint_calc(MODEM_CMUX_T3_TIMEOUT);
		modem_cmux_tx_bypass(cmux, wakeup_pattern, sizeof(wakeup_pattern));
		return true;
	}

	if (is_waking_up(cmux)) {
		if (sys_timepoint_expired(cmux->t3_timeout)) {
...
static void modem_cmux_transmit_handler(struct k_work *item)
{
	...
	if (transmit_rb_empty) {
		if (cmux->state == MODEM_CMUX_STATE_CONFIRM_POWERSAVE) {
			...
			if (MODEM_CMUX_PM_ENABLED && cmux->dev &&
			    pm_device_runtime_is_enabled(cmux->dev)) {
				pm_device_runtime_put_async(cmux->dev, K_NO_WAIT);
			}
		}
		modem_cmux_dlci_notify_transmit_idle(cmux);
...
static int modem_cmux_dlci_pipe_api_receive(void *data, uint8_t *buf, size_t size)
{
	...
	/* Release FC if set */
	if (dlci->rx_full && ring_buf_space_get(&dlci->receive_rb) >= MODEM_CMUX_DATA_FRAME_SIZE_MAX) {
		LOG_ERR("DLCI %u receive buffer is no longer full", dlci->dlci_address);
		dlci->rx_full = false;
		modem_cmux_send_msc(dlci->cmux, dlci);
	}
...
void modem_cmux_init(struct modem_cmux *cmux, const struct modem_cmux_config *config)
{
	...
	cmux->dev = config->dev;
	cmux->t3_timeout = sys_timepoint_calc(K_NO_WAIT);
...
int modem_cmux_attach(struct modem_cmux *cmux, struct modem_pipe *pipe)
{
	...
	if (MODEM_CMUX_PM_ENABLED && cmux->pipe && cmux->pipe->dev) {
		if (!pm_device_runtime_is_enabled(cmux->pipe->dev)) {
			ret = pm_device_runtime_enable(cmux->pipe->dev);
			LOG_INF("pm_device_runtime_enable(pipe): %d", ret);
		}
		cmux->runtime_pm = true;
		/* Attaching a pipe will wake up CMUX and its UART */
		pm_device_runtime_get(cmux->dev);
	}
...
void modem_cmux_release(struct modem_cmux *cmux)
{
	...
	/* Put CMUX to sleep, cannot use _async() because pipe references dropped */
	pm_device_runtime_put(cmux->dev);
	/* Unreference pipe */
	cmux->pipe = NULL;
...
int modem_cmux_pm_action(const struct device *dev,
				     enum pm_device_action action)
{
	struct modem_cmux *cmux = dev->data;
	LOG_DBG("modem_cmux_pm_action(%d) dev=%p cmux->dev = %p", action, dev, cmux->dev);

	if (action == PM_DEVICE_ACTION_SUSPEND) {
		if (!is_powersaving(cmux)) {
			LOG_ERR("Suspending CMUX instance while running (%p)", cmux->dev);
			if (IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME_USE_DEDICATED_WQ) && cmux->pipe->dev) {
				cmux->idle_timeout = sys_timepoint_calc(K_NO_WAIT);
				k_work_reschedule(&cmux->runtime_pm_work, K_NO_WAIT);
				bool ok = wait_state(cmux, MODEM_CMUX_STATE_POWERSAVE, MODEM_CMUX_T3_TIMEOUT);
				LOG_DBG("Suspended: %d", ok);
			} else {
				LOG_ERR("Refusing to suspend CMUX instance while running (%p)", cmux->dev);
				return -EBUSY;
			}
		}
		if (cmux->pipe && cmux->pipe->dev) {
			int ret = pm_device_runtime_put_async(cmux->pipe->dev, K_NO_WAIT);
			LOG_DBG("pm_device_runtime_put(%p): %d", cmux->pipe->dev, ret);
		}
		return 0;
	}
	...
	if (action == PM_DEVICE_ACTION_RESUME) {
		runtime_pm_keepalive(cmux);
		if (cmux->pipe && cmux->pipe->dev) {
			int ret = pm_device_runtime_get(cmux->pipe->dev);
			LOG_DBG("pm_device_runtime_get(%p): %d", cmux->pipe->dev, ret);
		}
		if (is_powersaving(cmux) || is_transitioning_to_powersave(cmux)) {
			/* Trigger wake up */
			k_work_schedule(&cmux->transmit_work, K_NO_WAIT);
		}
		return 0;
